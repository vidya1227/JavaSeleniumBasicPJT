...
public class ImageComparision_UsingShutteburg {
	...
	public void testImage() throws IOException, Exception {
		...
		Thread.sleep(2000);
		File image = new File("./Images/Edited.png"); // You have to check dimensions of images and you have resize
														// through paint as per actual image
		BufferedImage expectedImage = ImageIO.read(image);
		String OldImagePath = "./Images/ComparedImages/DifferedImage";

		boolean isImagesame = Shutterbug.shootPage(driver, ScrollStrategy.WHOLE_PAGE, 500, true).withName("Actual")
				.equalsWithDiff(expectedImage, OldImagePath, 0.1);
